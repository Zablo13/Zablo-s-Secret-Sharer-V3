...
decimal = [x for x in range(0, 100)]  # creates a list 0 - 99
...
codes_global = {}  # global variable to hold last generated or loaded codes
...
def save_codes_to_json(codes):
    filename = input("Enter filename to save codes (e.g., codes.json): ").strip()
...
def load_codes_from_json():
    filename = input("Enter filename to load codes from (e.g., codes.json): ").strip()
    try:
